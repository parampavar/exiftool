...
/// associated with it when the default [com.thebuzzmedia.exiftool.core.StandardFormat#NUMERIC]
/// value format is used.
///
/// The types provided by each tag are merely a hint based on the
/// [ExifTool Tag Guide](http://www.sno.phy.queensu.ca/~phil/exiftool/TagNames/index.html)
/// by Phil Harvey; the caller is free to parse or process the returned [String] values any way they wish.
///
...
/// cameras like simple point and shoots as well as DSLRs.
///
/// As an additional source of input the list of supported/common EXIF formats that Flickr
...
public enum StandardTag implements Tag {
	...
	/// specified by Phil Harvey's [ExifTool Tag Guide](http://www.sno.phy.queensu.ca/~phil/exiftool/TagNames/index.html).
