...
/// Perl-based [ExifTool](http://www.sno.phy.queensu.ca/~phil/exiftool).
///
...
/// creating an instance using [ExifToolBuilder]:
///
/// ```
...
/// ```
///
/// Once created, usage is as simple as making calls to [#getImageMeta(File, Collection)] or
/// [#getImageMeta(File, Format, Collection)] with a list of [Tag] you want to pull
...
///
/// Results from calls to [#getImageMeta] are returned in a [Map]
/// with the [com.thebuzzmedia.exiftool.core.StandardTag] values as the keys and [String] values for every
/// tag that had a value in the image file as the values. [com.thebuzzmedia.exiftool.core.StandardTag]s with no
/// value found in the image are omitted from the result map.
///
/// While each [com.thebuzzmedia.exiftool.core.StandardTag] provides a hint at which format the resulting value
/// for that tag is returned as from ExifTool (see [com.thebuzzmedia.exiftool.Tag#parse(String)]), that
/// only applies to values returned with an output format of
/// [com.thebuzzmedia.exiftool.core.StandardFormat#NUMERIC] and it is ultimately up to the caller to decide how
/// best to parse or convert the returned values.
///
/// The [com.thebuzzmedia.exiftool.core.StandardTag] Enum provides the [com.thebuzzmedia.exiftool.Tag#parse(String)]}
...
/// [Christian Etter discovered](http://www.christian-etter.de/?p=458) the overhead
/// for starting up a new Perl interpreter each time ExifTool is loaded accounts for
/// roughly [98.4% of the total runtime](http://u88.n24.queensu.ca/exiftool/forum/index.php/topic,1402.msg6121.html#msg6121).
/// Support for using ExifTool in daemon mode is enabled by explicitly calling
/// [ExifToolBuilder#enableStayOpen()] method.
///
/// Calling this method will create an instance of [ExifTool] with [com.thebuzzmedia.exiftool.core.strategies.StayOpenStrategy] execution strategy.
///
...
/// an [com.thebuzzmedia.exiftool.exceptions.UnsupportedFeatureException] if the native
/// ExifTool doesn't support the requested feature.
///
/// In the event of an [com.thebuzzmedia.exiftool.exceptions.UnsupportedFeatureException], the caller can either
...
///
/// When `stay_open` mode is used, there is the potential for
/// leaking both host OS processes (native `exiftool` processes) as well as the
/// read/write streams used to communicate with it unless [#close()] is
...
///
/// The inactivity period can be controlled by modifying the `exifTool.processCleanupDelay` system variable.
/// A value of `0` or less disabled the automatic cleanup process and requires you to cleanup
/// ExifTool instances on your own by calling [#close()] manually.
///
/// You can also set this delay manually using [com.thebuzzmedia.exiftool.ExifToolBuilder]:
///
/// ```
...
///
/// If you or the cleanup thread have called [#close()] on an instance of
/// this class, cleaning up the host process and read/write streams, the instance
/// of this class can still be safely used. Any followup calls to `getImageMeta` will
/// simply re-instantiate all the required resources necessary to service the call.
///
...
/// and when done, instead of [#close()]-ing the instance of this class and throwing it
...
/// [Pattern]s used to split the responses from the process are explicitly
/// compiled and reused, string concatenation is minimized, Tag name lookup is
/// done via a `static final` [Map] shared by all instances and
/// so on.
///
...
/// [ProcessBuilder#command(List)] method is used to avoid the copying of
/// array elements when [ProcessBuilder#command(String...)] is used and
/// avoiding the (hidden) use of [StringTokenizer] when
...
/// ### Thread Safety
///
/// Instances of this class **are Thread-safe** (note that version 1.1 of exiftool
/// was not Thread-safe):
...
/// ### Why ExifTool?
///
/// [ExifTool](http://www.sno.phy.queensu.ca/~phil/exiftool) is
...
/// ExifTool.
///
/// In addition, ExifTool already runs on all major platforms
...
/// [any image or video file](http://www.sno.phy.queensu.ca/~phil/exiftool/#supported) format.
///
/// ### Alternatives
///
...
/// across: [Metadata Extractor](http://drewnoakes.com/drewnoakes.com/code/exif/)
...
public class ExifTool implements AutoCloseable {
	...
	/// Cache used to store `exiftool` version:
	/// - Key is the path to the `exiftool` executable
	...
	/// an it an [UnsupportedFeatureException] will be thrown.
	///
	...
	/// instance of [ExifTool] from being re-used, it merely disposes of
	/// the native and internal resources until the next call to `getImageMeta`
	...
	/// [#close()] and this class will automatically re-create them on the
	/// next call to [#getImageMeta] if necessary.
	///
	/// @return `true` if there is an external ExifTool process is still
	...
	/// Exiftool. The user is also responsible for parsing  the raw output that has been returned.
	///
